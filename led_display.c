...
// Define GPIO Pins
#define P_RED RPI_GPIO_P1_19
#define P_GREEN RPI_GPIO_P1_21
#define P_BLUE RPI_GPIO_P1_23
#define P_PWR RPI_GPIO_P1_03
#define P_LATCH RPI_GPIO_P1_05
#define P_CLK RPI_GPIO_P1_07
...
//    bcm2835_set_debug(1);
	...
	bcm2835_gpio_fsel(P_RED, BCM2835_GPIO_FSEL_OUTP);
	bcm2835_gpio_fsel(P_GREEN, BCM2835_GPIO_FSEL_OUTP);
	bcm2835_gpio_fsel(P_BLUE, BCM2835_GPIO_FSEL_OUTP);
	bcm2835_gpio_fsel(P_PWR, BCM2835_GPIO_FSEL_OUTP);
	bcm2835_gpio_fsel(P_LATCH, BCM2835_GPIO_FSEL_OUTP);
	bcm2835_gpio_fsel(P_CLK, BCM2835_GPIO_FSEL_OUTP);
...
void pulseOnPwr()
{
	bcm2835_gpio_write(P_PWR, LOW);
	bcm2835_gpio_write(P_CLK, HIGH);
	bcm2835_gpio_write(P_CLK, LOW);
...
void pulseLatch()
{
	bcm2835_gpio_write(P_LATCH, LOW);
	bcm2835_gpio_write(P_LATCH, HIGH);
}
...
void setColors(const unsigned char col, const unsigned char row)
{
	...
	if(led[col][row][CLR_RED] > pwm){bcm2835_gpio_write(P_RED, HIGH);}
	else{bcm2835_gpio_write(P_RED, LOW);}
	
	if(led[col][row][CLR_GREEN] > pwm){bcm2835_gpio_write(P_GREEN, HIGH);}
	else{bcm2835_gpio_write(P_GREEN, LOW);}
	
	if(led[col][row][CLR_BLUE] > pwm){bcm2835_gpio_write(P_BLUE, HIGH);}
	else{bcm2835_gpio_write(P_BLUE, LOW);}
}
